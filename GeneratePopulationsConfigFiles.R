 ...
 for (popid in as.numeric(as.character(unique(dd$popid)))){
    ...
    name_gis_file_for_total_abundance_per_polygon <- c(paste("SpatialLayers/contour",popid,"_small_", dd[dd$popid==popid & dd$size=="small", "sizegroups"], sep=''), 
   ...
   a_comment <- popid
   namefile  <- file.path(general$main.path.param.gis, "POPULATIONS", "pops_config_files", paste(a_comment, "pops_creator_args_",  general$application, ".dat", sep=''))
