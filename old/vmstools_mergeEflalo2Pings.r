   ...
   retrieveOnBkSide <- function(merged, type.data){
      ...
      for(a.vesselid in all.vesselid){  # PER VESSEL
         ...
         logbk.this.vessel$VE_FLT     <- factor(logbk.this.vessel$VE_FLT)
           ...
           # mid time bk trips
           logbk.this.vessel <- logbk.this.vessel[!is.na(logbk.this.vessel$LE_DTIME),] # 18dec13
         ...
         # if does not exist, add SI_DATIM for handling the time in R
          tacsat.this.vessel$SI_DATE <- as.character(tacsat.this.vessel$SI_DATE)
          ...
          if(any(grep("/", tacsat.this.vessel$SI_DATE)))   format_date <- "%d/%m/%Y %H:%M"
          if(any(grep("-", tacsat.this.vessel$SI_DATE)))   format_date <- "%Y-%m-%d %H:%M" 
          if(length(strsplit(as.character(tacsat.this.vessel$SI_TIME[1]), split="\\:")[[1]])==3)   format_date <- paste(format_date, ":%S", sep='') 
          tacsat.this.vessel$SI_DATIM <- as.POSIXct(  paste(tacsat.this.vessel$SI_DATE, tacsat.this.vessel$SI_TIME), tz='GMT',   format_date)
