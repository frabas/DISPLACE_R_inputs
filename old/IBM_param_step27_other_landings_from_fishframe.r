 ...
 ICESrectangle <- function (dF) 
{
 ...
 ##---------------------------------------
  code_square_per_node <- read.table(file=file.path("C:", "Users", "fba", "Dropbox",
                             "ibm_vessels_param", "igraph", "code_square_for_graph_points.dat"), header=TRUE) # built in ibm_param_vessels.r
  ...
  ##---------------------------------------
   cat(paste("import landings data from fishframe (quarter/rectangle)\n",sep=''))

   # All country having upload into fishframe for 2010---------
   landings_all <- read.table(file=file.path(general$main.path.ibm, "fishframe",
                           'FF_CL_2010.csv'), sep=",", header=TRUE)

   colnames(landings_all) <- c("CL_CommercialLandingId", "LandingCountry", "country", "year", "quarter",        
                                "month", "sd", "rectangle", "species", "LandingCategory", "SizeCategoryScale", 
                                    "SizeCategory", "FishingActivityCategoryNational",
                                       "FishingActivityCategoryEuropeanL", "metier",
                                         "Harbour", "vessel_length", "UnallocatedCatchWeight",
                                           "AreaMisreportedCatchWeight", "landings", "LandingMultiplier",
                                              "OfficialLandingValue", "Stock")                           

   # get species fao code + add missing or mispelling species e.g. PRA and SAN
   load("C:\\Users\\fba\\Dropbox\\ibm_vessels_param\\IBM_datainput_speciesLatinNames.rda")
   ...
   landings_all$species_latin <- landings_all$species
   landings_all$species <- speciesLatinNames$fao_code[match(landings_all$species, speciesLatinNames$ff_species_latin)]
   
   landings <- landings_all[,c("country","year", "quarter", "month", "sd", "rectangle", "species",
                                  "metier", "vessel_length",  "landings", "Stock")]
  ...
  # first of all: keep 2010 only 
  landings <- landings[landings$year==general$a.year,]
  
  # add a semester
  landings$semester <- factor(landings$quarter); levels(landings$semester) <- c(1,1,2,2)

   # find out areas
   landings[,c('SI_LATI','SI_LONG')] <- ICESrectangle2LonLat(landings$rectangle, midpoint = TRUE)
   source(file=file.path("C:","Users","fba","Dropbox","ibm_vessels_param","IBM_param_utils_longlat_to_ICESareas.r"))
  ...
  # unfortunately cannot be exactly done because 
  # mismatch between VMS-equipped are not (< and >15m) and DCF segments (<10, 10-12,12-18, etc.)
  idx_to_remove <- which(landings$country %in% c('GER', 'DNK')  & landings$vessel_length %in% c('>40','18-<24','24-<40'))
  idx_to_keep   <- (1:nrow(landings))[-idx_to_remove]
  
   # here compute the respective part of landings which is
  # from simulated vessels vs. other landings (those recorded into fishframe that may be different than the ICES ones...pfff)
  ...
  pop_names                        <- read.table(file.path(general$main.path.ibm, "popsspe","pop_names.txt"))
  ...
  write.table(dd,    # ordered by species pop_names... 
               file= file.path(general$main.path.ibm, "popsspe", 
  ...
  DT$quarter<-as.factor(DT$quarter)
  DT$rectangle<-as.factor(DT$rectangle)
  DT$species<-as.factor(DT$species)
  land.agg <- DT[,eval(eq1),by=list(semester, rectangle, species)]
  land.agg <- data.frame( land.agg)
  colnames(land.agg) <- c( "semester", "rectangle", "species", nm[idx.col])
     ...
     for(semester in 1:2){
      ...
      cat(paste("semester ", semester, "\n"))
        ...
        from_avai_this_semester <- 
                   read.table(file.path(general$main.path.ibm,  "popsspe",
        ...
        if(nrow(landings_this_pop)!=0){
           ...
           # convert landings by semester into landings by month assuming equal ditribution 
        ...
        write.table(round(merged_this_species_this_quarter[, c('pt_graph', 'landings')]),
               file= file.path(general$main.path.ibm, "popsspe", 
                  paste(pop_names[pop,2], 'spe_oth_land_per_month_per_node_semester', semester, ".dat", sep='')), 
                 row.names=FALSE, col.names=TRUE, quote=FALSE)
