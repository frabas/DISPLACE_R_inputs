...
for (nmy in c("2014_2015", "2014")){
## CAUTION HERE: we need to run the script twice:
# 1: for the ibm runs: smoothed distribution over a period of several years
...
bits.cpue <- read.table(file.path(general$main_path, "IBM_datainput_DATRAS_CPUE_all_sp_per_length_per_haul_BITS_2014_2015.csv"), header=TRUE, sep=",")
ibts.cpue <- read.table(file.path(general$main_path, "IBM_datainput_DATRAS_CPUE_selected_sp_per_length_per_haul_NS-IBTS_2014_2015.csv"), header=TRUE, sep=",")
...
if(nmy=="2014") years <- c(2014)
if(nmy=="2014_2015") years <- c(2014:2015)
...
load(file.path(general$main_path, "merged_tables", general$casestudy, "all_merged_weight_DEN_2014.RData"))
...
# BITS
species.to.keep     <- c("COD", "PLE", "HER", "SPR", "WHG", "TUR", "FLE", "PLE", "DAB", "POK", "SOL", "HOM", "MON", "HKE", "HAD", 
                          "MUS", "OYF", "NEP")
...
save(bits.cpue, ibts.cpue, 
   file=file.path(general$main_path, "avai", paste("cpue_graph",general$igraph,"_",nmy,".RData",sep='')))
    ...
    cat(paste("szgroup", szgroup, "\n"))
     for(node in 1:nrow(coord)){
      #cat(paste("node", node, "\n"))
         ...
         savePlot(filename = file.path(general$main_path, "avai", paste("jpeg_igraph", general$igraph,"_", years,sep=''),
...
set.avai <- function(lst.avai, sp, S, areas){
     ...
     if(sp %in% c("SPR", "TUR", "DAB")){  # special case: west and east baltic merged
...
if(nmy=="2005_2010")  load(file.path(general$main_path, "avai",paste("cpue_graph",general$igraph,"_2005_2010.RData", sep='')))
if(nmy=="2010")       load(file.path(general$main_path, "avai",paste("cpue_graph",general$igraph,"_2010.RData", sep='')))
if(nmy=="2008_2012")  load(file.path(general$main_path, "avai",paste("cpue_graph",general$igraph,"_2008_2012.RData", sep='')))
if(nmy=="2012")       load(file.path(general$main_path, "avai",paste("cpue_graph",general$igraph,"_2012.RData", sep='')))
if(nmy=="2014_2015")  load(file.path(general$main_path, "avai",paste("cpue_graph",general$igraph,"_2014_2015.RData", sep='')))
if(nmy=="2014")       load(file.path(general$main_path, "avai",paste("cpue_graph",general$igraph,"_2014.RData", sep='')))
...
# input to DISPLACE_GUI
 ...
 idx                <- obj$Species %in% c("SPR", "TUR", "DAB") & obj$ICESareas %in% c("IVa", "IVb", "IVc") 
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'nsea', sep="_")
 idx                <- obj$Species %in% c("SPR",  "TUR", "DAB") & obj$ICESareas %in% c("IIIan","IIIas")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'kask', sep="_")
 idx                <- obj$Species %in% c("SPR",  "TUR", "DAB") & obj$ICESareas %in% c("IIIan","IIIas")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'kask', sep="_")
 idx                <- obj$Species %in% c("SPR",  "TUR", "DAB") & obj$ICESareas %in% c("22", "23", "24", "25", "26", "27","28-1","28-2","29","30","31","32")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], '2232', sep="_")

 idx                <- obj$Species %in% c("PLE") & obj$ICESareas %in% c("IVa", "IVb", "IVc", "IIIas") 
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'nsea', sep="_")
 idx                <- obj$Species %in% c("PLE") & obj$ICESareas %in% c("IIIas", "22", "23")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], '2123', sep="_")
 idx                <- obj$Species %in% c("PLE") & obj$ICESareas %in% c("24","25","26", "28-1", "28-2", "29","30","31","32")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], '2432', sep="_")

 
 idx                <- obj$Species %in% c("FLE") & obj$ICESareas %in% c("IVa", "IVb", "IVc") 
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'nsea', sep="_")
 idx                <- obj$Species %in% c("FLE") & obj$ICESareas %in% c("IIIan","IIIas")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'kask', sep="_")
 idx                <- obj$Species %in% c("FLE") & obj$ICESareas %in% c("22", "23")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], '2223', sep="_")
 idx                <- obj$Species %in% c("FLE") & obj$ICESareas %in% c("24","25")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], '2425', sep="_")
 idx                <- obj$Species %in% c("FLE") & obj$ICESareas %in% c("26", "28-1", "28-2")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], '2628', sep="_")
 idx                <- obj$Species %in% c("FLE") & obj$ICESareas %in% c("27", "29","30","31","32")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], '2732', sep="_")
 
 idx                <- obj$Species %in% c("SOL") & obj$ICESareas %in% c("IVa", "IVb", "IVc") 
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'nsea', sep="_")
 idx                <- obj$Species %in% c("SOL") & obj$ICESareas %in% c("IIIan","IIIas", "22", "23")
 obj[idx, "Stock" ] <- paste(obj[idx,"Species"], 'IIIa2223', sep="_")
 ...
 all_other_species  <- !obj$Species %in% c("SPR", "PLE", "FLE", "SOL", "TUR", "DAB", "COD", "HAD", "HER")
 ...
 #levels(obj$SizeGroup) <- gsub("nb_indiv.","",levels(obj$SizeGroup))
  ...
  write.table(obj, file=file.path(general$main_path, "avai",
           paste("survey_pop_distribution_on_points_ibts_",nmy,"_",general$igraph,".txt",sep='')), sep=";", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
get_contour_this_avai <- function (
     ...
     load(file = file.path(general$main_path, "avai",
                            paste("lst_avai_igraph", general$igraph,"_", nmy,"_",general$method,"_", general$threshold, ".RData",sep="")) )  # get lst.avai
