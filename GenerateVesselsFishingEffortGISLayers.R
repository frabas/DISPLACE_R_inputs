  ...
  if(ctry=="DEN"){
  load(file=file.path(general$main_path_gis, "FISHERIES", "coupled_VMS_logbooks_DNK_2015.RData")) # get the coupled_VMS_logbooks object
  ...
  cat(paste("Formatting for this country: TO DO! \n"))
   ### CHRISTIAN:  TO DO ###
  load(file=file.path(general$main_path_gis, "FISHERIES", "coupled_VMS_logbooks_DEU_2015.RData"))  # get the coupled_VMS_logbooks object
  ...
  aggResult$ctry   <- factor(aggResult$ctry)
  ...
  for (ctry in levels(aggResult$ctry)){
   ...
   # for (quarter in  unique(aggResult$quarter)){
     ...
     cat(paste("Create for",nameobj,".....done\n"))
